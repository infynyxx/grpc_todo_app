...
public class TodoClient {
  ...
  public void getToDoById(int id) {
    GetTodoRequest request = GetTodoRequest.newBuilder().setId(id).build();
    try {
      final Todo response = blockingStub.getTodoById(request);
      logger.info(
          String.format(
              "response: content=%s finished=%s", response.getContent(), response.getFinished()));
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    try {
      ...
      client.blockingStub.updateTodo(
          client
              .blockingStub
              .getTodoById(GetTodoRequest.newBuilder().setId(id).build())
              .toBuilder()
              .setContent("Prajwal")
              .build());
      client.getToDoById(id);
      client.blockingStub.updateTodo(
          Todo.newBuilder().setContent("hola").setId(randomUnsignedInt()).build());
    } finally {
      client.shutdown();
    }
  }
